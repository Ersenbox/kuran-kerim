...
const CACHE_NAME = 'kuran-v42';
const CACHE_VERSION = '2026-06-22-v42';
    ...
    caches.open(CACHE_NAME).then(cache => {
      ...
      return Promise.allSettled(
        CACHE_FILES.map(url => cache.add(url).catch(err => {
          console.warn('[SW v42] Cache skip:', url, err.message);
        }))
      );
    })
  );
...
self.addEventListener('activate', event => {
  console.log('[SW v42] Activating — clearing old caches...');
  event.waitUntil(
    caches.keys().then(keys => {
      return Promise.all(
        keys
          ...
          .map(key => {
            console.log('[SW v42] Deleting old cache:', key);
            return caches.delete(key);
          })
      );
    }).then(() => {
      console.log('[SW v42] Old caches cleared');
...
self.addEventListener('message', event => {
  ...
  if (event.data === 'CLEAR_CACHE') {
    ...
    console.log('[SW v42] All caches cleared by message');
  }
});
